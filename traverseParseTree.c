        ...
        for (int i = low_var; i <= high_var; i++, colon = colon_loc) {
            ...
            for (; colon < parseIndex; colon++) {
                if (strcmp(parseArray[colon]->symbolName, "R1")) {  // if not R1
                    ...
                    if (jd->parent < low) {
                        printf("Type Expression Error, Line Number: %3d, Statement Type: Declaration, ", getLineNumber(*t));
                        printf("Variables:");
    ...
    if (!strcmp(input->symbolName, "lhs")) {
        if (!strcmp(input->children[0]->symbolName, "VAR")) {
            if (input->children[1] == NULL) {
                ...
                while (rec) {
                    if(!strcmp(rec->children[0]->symbolName,"VAR"))
                        flag = false;
                    numNums++;
                    rec = rec->children[1];
                }
                ...
                if (temp->type == jagged_array){
                  if(temp->array_type == dyn){
                    ...
                    if (dimensions == 2) {
                        ...
                        if(s1>high_1){
                            snprintf(*msg, 200, "Type error as %d is greater than %d, high range of dimension 1", s1, high_1);
                            return NULL;
                        }
                        if(s1<low_1){
                            snprintf(*msg, 200, "Type error as %d is lower than %d, low range of dimension 1", s1, low_1);
                            return NULL;
                        }
                    ...
                    while(temp_rect!=NULL){
                        int dim_ = atoi(rec->children[0]->lexeme);
                        int high = atoi(temp_rect->high);
                        int low = atoi(temp_rect->low);
                        // printf("%d\n%d\n%d\n",dim_,high,low);
                        if(dim_>high){
                            snprintf(*msg,200,"Type error as %d is greater than %d, the high range of dimension %d\n",dim_,high,dimension);
                            return NULL;
                        }
                        if(dim_<low){
                            // printf("LESS\n");
                            snprintf(*msg,200,"Type error as %d is lower than %d, the low range of dimension %d\n",dim_,low,dimension);
                            return NULL;
                        }
              ...
              if (temp->type == jagged_array){
                ...
                if(!flag){
                  ...
                  if(dimensions == 2){
                      ...
                      if(s3>high_3){
                          snprintf(*msg, 200, "Type error as %d is greater than %d, high range of dimension 3", s3, high_3);
                          return NULL;
                      }
                      typeExpressionTable* temp_ = (typeExpressionTable*)malloc(sizeof(typeExpressionTable));
                      temp_->name = (char*)malloc(sizeof(char)*strlen("non_terminal"));
                ...
                if(!flag){
                  ...
                  while(temp_rect!=NULL){
                      ...
                      int low = atoi(temp_rect->low);
                      if(dim_>high){
                          snprintf(*msg, 200, "Type error as %d is lower than %d, the high range of dimension %d\n",dim_,high,dimension);
                          return NULL;
                      }
                      if(dim_<low){
                          snprintf(*msg,200,"Type error as %d is lower than %d, the low range of dimension %d\n",dim_,low,dimension);
...
void addAssignment(parseTree** t, typeExpressionTable* T) {
    ...
    if (!strcmp((*t)->symbolName, "assignment")) {
        ...
        if ((*t)->children[0]->type == NULL) {
            printf("Type Expression Error, Line Number: %3d, Statement Type: Assignment", getLineNumber(*t));
